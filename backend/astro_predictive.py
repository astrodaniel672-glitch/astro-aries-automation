...
try:
    ...
    from backend.astro_engine import NatalCalculationRequest, calculate_natal, _geo_place
    from backend.astro_rules import classify_aspect, proof_book
except ModuleNotFoundError:
    from astro_dignity import enhance_with_dignities
    from astro_engine import NatalCalculationRequest, calculate_natal, _geo_place
...
class PredictiveCalculationRequest(BaseModel):
    ...
    birth_time: str
    birth_place: str
    current_residence_place: str | None = None
...
TRANSIT_MAX_ORB = {"Sunce": 1.0, "Mesec": 1.0, "Merkur": 1.0, "Venera": 1.0, "Mars": 1.5, "Jupiter": 2.0, "Saturn": 2.0, "Uran": 1.5, "Neptun": 1.5, "Pluton": 1.5, "Severni čvor": 1.5}
...
def _sign_info(lon: float) -> dict[str, Any]:
    ...
    return {"longitude": round(lon, 6), "sign": sign, "sign_index": sign_index, "ruler": RULERS[sign], "degree_in_sign": round(deg_in_sign, 6), "degree": deg, "minute": minute, "second": second, "formatted": f"{deg}°{minute:02d}' {sign}"}
...
def _return_angles_houses(jd_ut: float, place_lat: float, place_lon: float, house_system: str) -> dict[str, Any]:
    try:
        cusps, ascmc = swe.houses_ex(jd_ut, place_lat, place_lon, house_system.encode("ascii"))
        return {"angles": {"ASC": _sign_info(ascmc[0]), "MC": _sign_info(ascmc[1]), "DSC": _sign_info(ascmc[0] + 180), "IC": _sign_info(ascmc[1] + 180)}, "houses": [_sign_info(cusps[i]) | {"house": i + 1} for i in range(12)]}
    except Exception as exc:
        return {"angles": {}, "houses": [], "house_calculation_error": str(exc)}


def _prediction_place(request: PredictiveCalculationRequest, natal: dict[str, Any]) -> dict[str, Any]:
    if request.current_residence_place and request.current_residence_place.strip():
        place = _geo_place(request.current_residence_place.strip())
        place["role"] = "current_residence_place"
        place["relocation_rule"] = "Prediction return charts use current residence place; natal chart remains calculated for birth place."
        return place
    place = dict(natal.get("place") or {})
    place["role"] = "birth_place_fallback"
    place["relocation_rule"] = "No current_residence_place supplied; prediction return charts fall back to birth place for backward compatibility."
    return place
...
def _aspect_row(method: str, moving_name: str, moving_lon: float, natal_name: str, natal_lon: float, aspect_name: str, orb: float, exact_jd: float | None = None) -> dict[str, Any]:
    row = {"method": method, "point_a": moving_name, "point_b": natal_name, "group_a": "predictive", "group_b": "natal_point", "aspect": aspect_name, "angle": ASPECT_ANGLES[aspect_name], "orb": round(orb, 4), "exactness": "tight" if orb <= 1 else "normal", "moving_position": _sign_info(moving_lon), "natal_position": _sign_info(natal_lon)}
    if exact_jd:
    ...
    if classified.get("orb") <= 0.1667:
        classified["evidence_weight"] = "primary_evidence"; classified["orb_class"] = "exact"; classified["usable_as_proof"] = True
    elif classified.get("orb") <= 1.0:
        classified["evidence_weight"] = "strong_evidence"; classified["orb_class"] = "tight"; classified["usable_as_proof"] = True
    elif classified.get("orb") <= 2.0 and aspect_name in {"konjunkcija", "opozicija", "kvadrat", "kvinkunks"}:
        classified["evidence_weight"] = "supporting_evidence"; classified["orb_class"] = "valid"; classified["usable_as_proof"] = True
    else:
        classified["evidence_weight"] = "background_only"; classified["usable_as_proof"] = False
    ...
    for planet in TRANSIT_PLANETS:
        ...
        for natal_name, natal_data in natal_points.items():
            ...
            if natal_lon is None:
                continue
            for aspect_name, angle in ASPECT_ANGLES.items():
                previous = None; previous_jd = None; closest: dict[str, Any] | None = None
                for day in range(0, total_days + 1, step):
                    ...
                    abs_delta = abs(delta)
                    if abs_delta <= max_orb and (closest is None or abs_delta < closest["orb"]):
                        closest = {"jd": jd_now, "lon": lon, "orb": abs_delta}
                    if previous is not None and (delta == 0 or previous * delta < 0):
                    ...
                    previous = delta; previous_jd = jd_now
...
def _solar_return(natal: dict[str, Any], birth_year: int, target_year: int, prediction_place: dict[str, Any], house_system: str) -> dict[str, Any]:
    ...
    loc = _return_angles_houses(refined["jd"], prediction_place["latitude"], prediction_place["longitude"], house_system)
    return {"method": "solar_return", "target_year": target_year, "location": prediction_place, "relocation_note": "Solar return houses/angles are calculated for current_residence_place when supplied; otherwise birth_place fallback is used.", "exact_utc": _revjul_iso(refined["jd"]), "julian_day_ut": round(refined["jd"], 8), "sun_return_orb": round(refined["orb"], 6), "angles": loc.get("angles", {}), "houses": loc.get("houses", []), "house_calculation_error": loc.get("house_calculation_error"), "planets": _planet_positions(refined["jd"])}
...
def _secondary_progressions(natal: dict[str, Any], birth_dt_utc: datetime, calc_dt: datetime, natal_points: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"method": "secondary_progressions_day_for_year", "calculation_utc": calc_dt.isoformat(), "progressed_utc_symbolic": progressed_dt.isoformat(), "progressed_julian_day_ut": round(progressed_jd, 8), "planets": planets, "contacts_to_natal": _contacts_at_jd("secondary_progression_to_natal", planets, natal_points, 2.0)}
...
def _solar_arc(natal: dict[str, Any], birth_dt_utc: datetime, calc_dt: datetime, natal_points: dict[str, Any]) -> dict[str, Any]:
    ...
    for name, data in natal.get("angles", {}).items():
        if name in ANGLE_TARGETS and "longitude" in data:
            directed[name] = _sign_info(data["longitude"] + arc)
    return {"method": "solar_arc_directions", "calculation_utc": calc_dt.isoformat(), "arc_degrees": round(arc, 6), "directed_points": directed, "contacts_to_natal": _contacts_at_jd("solar_arc_to_natal", directed, natal_points, 2.0)}


def _lunar_returns(start: datetime, end: datetime, natal: dict[str, Any], prediction_place: dict[str, Any], house_system: str) -> list[dict[str, Any]]:
    natal_moon = natal["planets"]["Mesec"]["longitude"]
    rows: list[dict[str, Any]] = []
    jd_start = _jd(start); jd_end = _jd(end); jd_now = jd_start
    previous_delta = None; previous_jd = None
    while jd_now <= jd_end:
        ...
        if previous_delta is not None and previous_delta * delta < 0:
            ...
            loc = _return_angles_houses(exact_jd, prediction_place["latitude"], prediction_place["longitude"], house_system)
            rows.append({"method": "lunar_return", "location": prediction_place, "relocation_note": "Lunar return houses/angles are calculated for current_residence_place when supplied; otherwise birth_place fallback is used.", "exact_utc": _revjul_iso(exact_jd), "julian_day_ut": round(exact_jd, 8), "moon_position": _sign_info(exact_moon), "orb": round(_diff(exact_moon, natal_moon), 4), "angles": loc.get("angles", {}), "houses": loc.get("houses", []), "house_calculation_error": loc.get("house_calculation_error")})
        previous_delta = delta; previous_jd = jd_now; jd_now += 1.0
    return rows[:24]
...
def _prediction_proof_book(predictive: dict[str, Any]) -> dict[str, Any]:
    sets: dict[str, list[dict[str, Any]]] = {"transits": predictive.get("transits_to_natal", {}).get("window_hits", []), "secondary_progressions": predictive.get("secondary_progressions", {}).get("contacts_to_natal", []), "solar_arc": predictive.get("solar_arc", {}).get("contacts_to_natal", [])}
    return proof_book(sets)


def calculate_predictive(request: PredictiveCalculationRequest) -> dict[str, Any]:
    natal_request = NatalCalculationRequest(birth_date=request.birth_date, birth_time=request.birth_time, birth_place=request.birth_place, calculation_date=request.calculation_date or request.prediction_start, house_system=request.house_system, zodiac=request.zodiac)
    ...
    prediction_place = _prediction_place(request, natal)
    ...
    sr = _solar_return(natal, birth_dt_utc.year, target_year, prediction_place, request.house_system)
    ...
    lunars = _lunar_returns(start, min(end, start + timedelta(days=370)), natal, prediction_place, request.house_system)
    ...
    result = {
        ...
        "settings": {
            ...
            "birth_place": natal.get("place"),
            "current_residence_place": prediction_place,
            "relocation_rule": "Natal chart uses birth_place. Solar return and lunar returns use current_residence_place when supplied; otherwise they use birth_place fallback.",
        ...
        "solar_arc": solar_arc,
        "transits_to_natal": {"period_start_positions": transits_now, "contacts_at_period_start": transit_contacts_now, "window_hits": window_hits},
        ...
        "quality_warnings": [
            ...
            "Solar return i lunar returns sada koriste current_residence_place kada je unet; ako nije unet, koriste birth_place fallback.",
        ],
    }
